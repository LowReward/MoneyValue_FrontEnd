<template>
    <div>
        ...
        <div class="form-group">
            ...
            <select id="pairFrom" class="form-control" v-model="selectedPairFrom" required>
        ...
        <div class="form-group">
            ...
            <select id="pairTo" class="form-control" v-model="selectedPairTo" required>
                <!-- Boucle sur les devises uniques disponibles -->
                <option v-for="currency in uniqueCurrenciesTo" :key="currency" :value="currency" >
                    {{ currency }}
                </option>
            </select>
        </div>
...
export default {
    ...
    computed: {
        ...
        uniqueCurrenciesTo() {
            const currencies = new Set(); // Ensemble pour stocker les devises de retour uniques
            for (const pair of this.pairs) {
                currencies.add(pair.currency_to); // Ajout de chaque devise de retour dans l'ensemble
            }
            return Array.from(currencies); // Conversion de l'ensemble en tableau pour l'affichage dans le sélecteur
        },
    },
